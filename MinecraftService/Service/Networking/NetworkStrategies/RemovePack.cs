...
namespace MinecraftService.Service.Networking.NetworkStrategies {
    public class RemovePack(ServiceConfigurator serviceConfiguration, MmsLogger logger) : IMessageParser {
        ...
        public Message ParseMessage(Message message) {
            ...
            string worldDirectory = $@"{serviceConfiguration.GetServerInfoByIndex(message.ServerIndex).GetSettingsProp(ServerPropertyKeys.ServerPath)}\worlds\{serviceConfiguration.GetServerInfoByIndex(message.ServerIndex).GetProp(MmsDependServerPropKeys.LevelName)}";
